...
class TCA(nn.Module):
    def __init__(self, c, d=1, dropout=0, kSize=3, dkSize=3):
        ...
        self.dconv3x3 = Conv(
            c, c, (dkSize, dkSize), 1, padding=(1, 1), groups=c, bn_acti=True
        )
...
class LCNet(nn.Module):
    def __init__(self, num_classes=19, block_1=3, block_2=7, C=32, P=0.5):
        super().__init__()
        ...
        for i in range(0, block_1):
            ...
            self.LC_Block_1.add_module(
                "LC_Module_1_" + str(i), PCT(nIn=C * 2, d=dilation_block_1[i], p=P)
            )
        ...
        for i in range(0, block_2):
            ...
            self.LC_Block_2.add_module(
                "LC_Module_2_" + str(i), PCT(nIn=C * 4, d=dilation_block_2[i], p=P)
            )
        self.DAD = DAD(C * 4, C * 2, num_classes)
...
class DAD(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        F_rg = F.interpolate(
            F_rg, F_sg.size()[2:], mode="bilinear", align_corners=False
        )
...
if __name__ == "__main__":
    ...
    model = LCNet(num_classes=6).to(device)
    x = torch.randn(2, 3, 256, 256).to(device)
    assert model(x).shape == (2, 6, 256, 256)