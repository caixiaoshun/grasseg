...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start:th.Tensor, t, model_kwargs=None, noise=None):
        """
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start.float())
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs["betas"] = get_named_beta_schedule(noise_schedule, steps)
        kwargs["model_mean_type"] = (
            ModelMeanType.EPSILON if not predict_xstart else ModelMeanType.START_X
        )
        kwargs["model_var_type"]=(
            (
                ModelVarType.FIXED_LARGE
                if not sigma_small
