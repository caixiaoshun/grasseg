...
class DualResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer3_1 = self._make_layer(
            block, planes * 2, planes * 4, layers[2] // 2, stride=2
        )
        ...
        self.layer3_1_ = self._make_layer(
            block, planes * 2, highres_planes, layers[2] // 2
        )

        self.layer3_2_ = self._make_layer(
            block, highres_planes, highres_planes, layers[2] // 2
        )

        self.layer4_ = self._make_layer(
            block, highres_planes, highres_planes, layers[3]
        )
...
if __name__ == "__main__":
    ...
    model = DualResNet(num_classes=6).to(device)
