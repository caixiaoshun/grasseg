...
def give_colors_to_mask(image: np.ndarray, mask: np.ndarray, num_classes=6,colors = None)->np.ndarray:
    """将mask转换为彩色

    Args:
        image (np.ndarray): np.ndarray, (H, W, 3) dtype: uint8
        mask (np.ndarray): np.ndarray, (H, W)    dtype: uint8
        num_classes (int, optional): num classes of mask. Defaults to 6.
